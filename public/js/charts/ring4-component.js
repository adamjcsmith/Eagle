...
function prepareRing(maxMark, studentData, dataTrack, socketinput) {
	...
	if (!window.PointerEvent) {
	  // Mouse Events Only:
		...
		// Touch events (Windows) supported:
		canvas.addEventListener("touchstart", eventTouchStart, false); 
		canvas.addEventListener("touchmove", eventTouchMove, true); 
		canvas.addEventListener("touchend", eventTouchEnd, false); 
		canvas.addEventListener("pointerdown", eventMouseDown, false); 
		canvas.addEventListener("pointermove", eventMouseMove, false); 
		canvas.addEventListener("pointerup", eventMouseUp, false); 
		canvas.addEventListener("pointerleave", eventMouseLeave, false);	 
		canvas.addEventListener("pointerover", eventMouseOver, false); 				
	}
		...
		for(var i=0; i<dataTracker.length; i++) {
			...
			if( (Math.pow(xDiff,2) + Math.pow(yDiff,2)) < Math.pow(radius,2)) {
				...
				socket.on('studentResponse', function(data) {
					...
					document.getElementById('insightMark').innerHTML = "etc!";
...
function removeInsightMarks(context, canvas) {
	for(var i=0; i<dataTracker.length; i++) {
		if(dataTracker[i].insightMark == true)	dataTracker.splice(i, 1);
	}
...
function checkDataByID(id) {
	for(var i=0; i<dataTracker.length; i++) {
		if(dataTracker[i].originalID == id)	return true;
	}
	return false;
}
...
function addMinMaxLine(context) {
	...
	// Find a student's minimum and maximum marks this year:
	for(var i=0; i<dataTracker.length; i++) {
...
function lockTier(label, mode) {
	var minThreshold = 0;
	var maxTreshold = 0;
	if(label == 'Fail') { minThreshold = 0; maxThreshold = 40; }
	else if(label == '3rd') { minThreshold = 40; maxThreshold = 50; }
	else if(label == '2:2') { minThreshold = 50; maxThreshold = 60; }
	else if(label == '2:1') { minThreshold = 60; maxThreshold = 70; }
	else if(label == '1:1') { minThreshold = 70; maxThreshold = 100; }
